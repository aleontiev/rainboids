...
export class AbilityManager {
  ...
  handleAutoplayAbilities(collidables, keys, powerups = [], game = null) {
    ...
    const enemyBullets = collidables.filter(c => c.collidableType === 'enemyBullet');
    const enemyLasers = collidables.filter(c => c.collidableType === 'enemyLaser');
    ...
    const collisionPredictions = this.autoplayer.predictAllCollisions(allThreats, 1.0);
    
    // Check if movement alone can avoid all imminent collisions
    const unavoidableCollisions = this.autoplayer.findUnavoidableCollisions(collisionPredictions, allThreats);
    ...
    const shouldUseShield = (
      (unavoidableCollisions.length > 0 && unavoidableCollisions[0].prediction.timeToCollision < 45) || // Unavoidable collision imminent
      (threatLevel >= (0.8 * bossModifier)) || // High threat level
      (nearbyProjectiles >= Math.floor(6 * bossModifier) && veryCloseProjectiles >= 2) || // Many close projectiles
      (nearCornerShield && nearbyProjectiles >= Math.floor(4 * bossModifier) && threatLevel >= (0.6 * bossModifier)) || // Cornered with threats
    ...
    const shouldUseTimeSlow = (
      ...
      (nearbyProjectiles >= Math.floor(8 * bossModifier) && veryCloseProjectiles >= Math.floor(3 * bossModifier)) || // Many close projectiles
      (nearCornerSlow && nearbyProjectiles >= Math.floor(6 * bossModifier) && threatLevel >= (0.7 * bossModifier)) || // Cornered
    ...
    if (shouldUseTimeSlow && 
        ...
        game && game.timeSlowCooldown <= 0 && !game.timeSlowActive) {
      keys.f = true;
      this.autoplaySlowTimer = 0;
    }
  ...
  detectBossPhaseChange(boss) {
    return this.threatAssessment.detectBossPhaseChange(boss, this.lastBossPhase, this.lastBossArmsDestroyed);
  }
