...
export class MiniBoss {
  constructor(x, y, type, isPortrait, canvasWidth = 800) {
    ...
    this.godMode = true;
    this.godModeTimer = 300; // 5 seconds at 60fps
  ...
  render(ctx) {
    ...
    // Godmode shield visual
    if (this.godMode) {
      ctx.strokeStyle = "#00ffff"; // Cyan color for godmode shield
      ctx.lineWidth = 5;
      ctx.globalAlpha = 0.7 + 0.3 * Math.sin(this.frameCount * 0.2); // Pulsing effect
      ctx.beginPath();
...
export class Level1Boss {
  constructor(x, y, isPortrait, canvasWidth, canvasHeight) {
    ...
    this.maxHealth = 3000; // Very high health
    this.health = this.maxHealth;
    this.frameCount = 0;

    // Shields
    this.shield1Health = 1000; // Health for the first shield
    this.shield2Health = 1000; // Health for the second shield
    this.currentShield = 1; // 1 for first, 2 for second, 0 for no shield
  ...
  update(slowdownFactor = 1.0) {
    ...
    if (this.movePattern === "entering") {
      ...
      if (distance > 5) {
        this.x += (dx / distance) * this.speed * slowdownFactor;
        this.y += (dy / distance) * this.speed * slowdownFactor;
      } else {
        this.movePattern = "patrol";
        this.startX = this.x;
        this.startY = this.y;
      ...
      // Patrol movement
      if (this.isPortrait) {
        this.x += this.patrolDirection * 0.5 * slowdownFactor;
        if (Math.abs(this.x - this.startX) > this.patrolRange) {
          this.patrolDirection *= -1;
        }
      } else {
        this.y += this.patrolDirection * 0.5 * slowdownFactor;
        if (Math.abs(this.y - this.startY) > this.patrolRange) {
          this.patrolDirection *= -1;
        }
      }
    ...
    // Phase management
    this.phaseTimer += slowdownFactor;
    ...
    this.primaryWeaponTimer += slowdownFactor;
    this.secondaryWeaponTimer += slowdownFactor;
    this.specialWeaponTimer += slowdownFactor;
    this.spiralWeaponTimer += slowdownFactor;
  ...
  takeDamage(damage) {
    if (this.currentShield === 1) {
      this.shield1Health -= damage;
      if (this.shield1Health <= 0) {
        this.currentShield = 2;
        this.health -= this.shield1Health; // Apply any overflow damage to main health
        return "shield1_destroyed";
      }
    } else if (this.currentShield === 2) {
      this.shield2Health -= damage;
      if (this.shield2Health <= 0) {
        this.currentShield = 0;
        this.health -= this.shield2Health; // Apply any overflow damage to main health
        return "shield2_destroyed";
      }
    } else {
      this.health -= damage;
    }
  ...
  getHealthPercentage() {
    if (this.currentShield === 1) {
      return this.shield1Health / 1000; // Max shield 1 health
    } else if (this.currentShield === 2) {
      return this.shield2Health / 1000; // Max shield 2 health
    } else {
  ...
  render(ctx) {
    ...
    // Render shields
    if (this.currentShield === 1) {
      ctx.strokeStyle = "#00aaff"; // Blue for first shield
      ctx.lineWidth = 8;
      ctx.globalAlpha = 0.8;
      ctx.beginPath();
      ctx.arc(0, 0, this.size + 25, 0, Math.PI * 2);
      ctx.stroke();
    } else if (this.currentShield === 2) {
      ctx.strokeStyle = "#ffaa00"; // Orange for second shield
      ctx.lineWidth = 8;
      ctx.globalAlpha = 0.8;
      ctx.beginPath();
      ctx.arc(0, 0, this.size + 25, 0, Math.PI * 2);
  ...
  renderHealthBar(ctx) {
    ...
    if (this.currentShield === 1) {
      healthPercent = this.shield1Health / 1000;
      barColor = "#00aaff"; // Blue for first shield
    } else if (this.currentShield === 2) {
      healthPercent = this.shield2Health / 1000;
      barColor = "#ffaa00"; // Orange for second shield
    } else {
